...
resource "aws_lb_listener_rule" "redirect_map_portal" {
  ...
  action {
    ...
    redirect {
      host        = var.map_portal.hostname
      path        = var.map_portal.path
      port        = "443"
      protocol    = "HTTPS"
  ...
  condition {
    host_header {
      values = [var.r53_public_hosted_zone]
    }
  }
}
...
resource "aws_ecs_service" "app" {
  ...
  network_configuration {
    ...
    subnets         = aws_vpc.private_subnet_ids
  }
