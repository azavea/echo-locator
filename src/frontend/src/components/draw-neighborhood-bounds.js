...
import { NEIGHBORHOOD_BOUNDS_STYLE, NEIGHBORHOOD_ROUTABLE_BOUNDS_STYLE } from "../constants";
...
class DrawNeighborhoodBounds extends React.PureComponent {
  ...
  getTooltip = (feature, t) => {
    ...
    const tooltipClass = routable
      ? "map__tooltip-town"
      : "map__tooltip-town map__tooltip-town--unroutable";
    tooltipEl.setAttribute("class", tooltipClass);
    const townText = document.createTextNode(
      routable ? town : town + " (" + t("Map.Unreachable") + ")"
    );
  ...
  render() {
    ...
    return (
      <LayerGroup key={`neighborhood-boundary-${this._getKey()}`}>
        {p.neighborhoods && (
          <VGrid
