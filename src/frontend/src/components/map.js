...
import type { ActiveListing, Coordinate, Location, LonLat, MapEvent, Listing } from "../types";
...
const TILE_URL =
  Leaflet.Browser.retina && process.env.LEAFLET_RETINA_URL
    ? process.env.LEAFLET_RETINA_URL
    : process.env.LEAFLET_TILE_URL;

const LABEL_URL =
  Leaflet.Browser.retina && process.env.LABEL_RETINA_URL
...
class Map extends PureComponent<Props, State> {
  ...
  handleSetActiveListing = (event, detail) => {
    if (detail) {
      ...
      if (event.type === "mouseover") {
        this.setState({
          listingRouteDelay: setTimeout(() => this.props.setActiveListing(listingDetail), 500),
        });
  ...
  */
  popupDetailOnHover = (data) => {
    const { photos, rent, beds, address } = data;
    const { t, i18n } = this.props;
    const popupWidth = 240;
    return (
      <div className="map__popup" style={{ width: popupWidth }}>
        {photos && photos.length > 0 && (
          <img
            className="map__popup-image"
            src={photos[0].href}
            key={`listings-image-${this._getKey()}`}
          />
        )}
        <div className="map__popup-details">
          {address && <div className="map__popup-address">{address}</div>}
          {rent && (
            <div className="map__popup-rent">
              {t("Map.PopupDetails.RentAmount", { rent: rent.toLocaleString(i18n.language) })}
            </div>
          )}
          {beds && (
            <div className="map__popup-beds">
              {beds.split(" ")[0] + " " + t("NeighborhoodDetails.BedroomAbbr")}
            </div>
          )}
  ...
  render() {
    ...
    const listingsMarker = (data) => {
      ...
      return (
        ...
        >
          <Popup autoPan={false} closeButton={false} className="listing-detail-popup">
            {popupDetailOnHover(data)}
          </Popup>
        </Marker>
      );
    ...
    return p.routableNeighborhoods ? (
      ...
      >
        ...
        <TileLayer {...TILE_OPTIONS} url={TILE_URL} zIndex={getZIndex()} />
        ...
        {!p.isLoading && p.activeNeighborhoodBounds && (
          <VGrid
            ...
            vectorTileLayerStyles={{ sliced: NEIGHBORHOOD_ACTIVE_BOUNDS_STYLE }}
            zIndex={getZIndex()}
          />
        )}
        ...
        {p.showRealtorListings &&
          p.realtorListings.data &&
          p.realtorListings.data.map((item, key) =>
            createMarkerWithStandardizedData(item, "Realtor")
          )}

        {p.showBHAListings &&
          p.bhaListings.data &&
          p.bhaListings.data.map((item, key) => createMarkerWithStandardizedData(item, "BHA"))}

        {!p.showDetails &&
          p.displayNeighborhoods &&
          p.displayNeighborhoods.length &&
