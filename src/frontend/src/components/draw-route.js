...
export default class DrawRoute extends React.PureComponent {
  componentWillReceiveProps(nextProps) {
    ...
    if (!layer || (nextProps.activeNeighborhood && nextProps.activeNeighborhood !== nextProps.id)) {
      return;
    }
  ...
  render() {
    ...
    return (
      <FeatureGroup ref="features">
          ...
          if (segment.type === "WALK") {
            ...
            return (
              <Polyline
                ...
                positions={segment.positions}
                color={
                  !this.props.hasVehicle && this.props.showDetails
                    ? segment.color
                    : p.transitStyle.color
                }
              />
            );
          }
        ...
        {p.stops.map((s, i) => (
          <CircleMarker className="Stop" key={`stop-${i}`} center={s} {...p.stopStyle} />
        ))}
      </FeatureGroup>
    );
  }
