...
export const setNetworksAccessibilityTo =
  (value: string) => (dispatch: Dispatch, getState: any) => {
...
export const setNetworksToLoading = () => setNetworksAccessibilityTo(ACCESSIBILITY_IS_LOADING);
export const setNetworksToEmpty = () => setNetworksAccessibilityTo(ACCESSIBILITY_IS_EMPTY);
...
export const initialize = (startCoordinate?: LonLat) => (dispatch, getState) => {
  ...
  const start =
    startCoordinate ||
    (state && state.data && state.data.origin ? state.data.origin.position : null);
  if (process.env.DISABLE_CONFIG) {
    ...
    dispatch(
      loadDataset(state.data.networks, state.data.grids, state.data.pointsOfInterestUrl, start)
    );
  } else {
    try {
      ...
      if (json) {
        if (!json.networks) {
          ...
          return dispatch(loadDataset(json.networks, json.grids, json.pointsOfInterestUrl, start));
        }
      }
    } catch (e) {
...
export const fetchAllTimesAndPathsForIndex =
...
const fetchTimesAndPathsForNetworkAtCoordinate = (network, coordinate, currentZoom) => {
  const originPoint = coordinateToPoint(coordinate, network.zoom, network.west, network.north);
  ...
  return !isNaN(index)
    ...
    : [
        logItem(
          `Skipping data fetch for invalid origin (x: ${originPoint.x}, y: ${originPoint.y})`
        ),
      ];
};
