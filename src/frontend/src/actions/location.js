...
import { fetchAllTimesAndPathsForCoordinate, setNetworksToLoading } from "./network";
...
export const updateOrigin = (value?: Location) =>
  value && value.label && value.position
    ...
    : [addActionLogItem("Clearing origin"), setOrigin()];
...
export const updateStart = (value?: Location) =>
  value && value.label && value.position
    ...
    : [addActionLogItem("Clearing start"), setStart()];

export const updateStartPosition = (position: LonLat) => [
  reverseGeocode(position, (features) => setStart({ position, label: features[0].place_name })),
...
export const updateEndPosition = (position: LonLat) =>
  reverseGeocode(position, (features) => setEnd({ position, label: features[0].place_name }));
