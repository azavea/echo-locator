...
export default function createTransitiveRoutesForNetwork(
  ...
  const paths = targetPathIndexes.map((tpi) =>
    network.paths[tpi]
  ...
  const journeys = paths.map((path) => ({
    journey_id: 0,
    journey_name: 0,
    segments: path.length === 0 ? createWalkOnlyJourney() : getTransitiveSegmentsFromPath(path),
  }));
  ...
  return {
    ...
    routeSegments: paths.map((path) =>
      path.map((leg) => {
        ...
        const seg = {};
        const getRouteName = (r) => toUpperCase(route.route_short_name);
        const color = route.route_color ? `#${route.route_color}` : "#0b2b40";
