...
This file is provided by BHA. Each row represents a neighborhood, therefore each zip code field should be a unique value. A README.txt file is at the root of the `neighborhood_data` directory with additional data type details and context for CSV columns. When sending the CSV template to BHA for neighborhood data updates we should also include this README file as a reference.
...
> **_NOTE:_**  If running the `update_data` script within a Python virtual environment (venv) it may fail with an SSL error on downloading data from www2.census.gov. In that scenario, download the ZCTA data manually (https://www2.census.gov/geo/tiger/GENZ2020/shp/cb_2020_us_zcta520_500k.zip) and ensure zip in the `neighborhood_data/zctas` directory.

If any images downloads are skipped with an "original image is less than 640px wide" message you will need to manually download the image instead. Follow that image's wikimedia link and select the next image size available greater than 640px wide. Once downloaded place in `neighborhood_data/images/` dir and re-run `./update_data.sh` to update the neighborhood json. 

The downloaded images need to be deployed separately from the other app data.
To publish the neighborhood images:
