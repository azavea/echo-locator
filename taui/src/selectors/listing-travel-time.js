...
export default createSelector(
  ...
  (activeNetworkIndex, surfaces, networks, activeListing, origin) => {
    const network = networks[activeNetworkIndex]
    const surface = surfaces[activeNetworkIndex]
    if (!network || !network.ready || !surface || !origin || !activeListing) {
      return 0
    }
    const idx = coordinateToIndex(lonlat(activeListing), network)
