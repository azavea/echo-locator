    ...
    return new Promise (resolve => {
        ...
        return Promise.all(promises).then((results) => {
            for(var i = 0; i < results.length; i++) {
              ...
              response[i].address = {city: response[i].City, country: "USA", lat: response[i].latLon.lat, line: response[i]['Street Number'].concat(' ').concat(response[i]['Street Name']), 
                lon: response[i].latLon.lng, postal_code: response[i]['ZIP CODE (xxxxx)'], state: "Massachusetts", state_code: "MA", time_zone: "America/New_York"}
