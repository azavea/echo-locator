  ...
  return axios.get(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values:batchGet?ranges=Sheet1&majorDimension=ROWS&key=${process.env.GSHEET_API_KEY}`)
    .then(response => {
      ...
      for (let i = 1; i < batchRowValues.length; i++) {
        ...
        //Getting street address to pass into fwdGeoCode for latlon
        rowObject['geoAddress'] = rowObject['Street Number'].concat(' ').concat(rowObject['Street Name']).concat(', ').concat(rowObject.City).concat(', MA')


        //URL of this listing on BHA website
        rowObject['rdc_web_url'] = 'https://www.bostonhousing.org/en/Appartment-Details.aspx?item='.concat(rowObject['kenti'])

        //Array of image urls on BHA website based on Thumb Image header in spreadsheet
        rowObject['photos'] = new Array()
        var count = 1
        var thumbImage = 'Thumb Image'.concat(count)
        while (rowObject[thumbImage]) {
          rowObject['photos'].push({href: 'https://www.bostonhousing.org/ApartmentImage/'.concat(rowObject[thumbImage])})
          count += 1
          thumbImage = 'Thumb Image'.concat(count)

        }

        // Add this rowObject to the ones we return if the zip code is right, the number of
      ...
      return Promise.all(promises).then((results) => {
        for(var i = 0; i < results.length; i++) {
          ...
          rows[i].address = {city: rows[i].City, country: "USA", lat: rows[i].latLon.lat, line: rows[i]['Street Number'].concat(' ').concat(rows[i]['Street Name']), 
            lon: rows[i].latLon.lng, postal_code: rows[i]['ZIP CODE (xxxxx)'], state: "Massachusetts", state_code: "MA", time_zone: "America/New_York"}
        }
        return rows
      })
    })
